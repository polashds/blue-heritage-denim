    ...
    <>
      ...
      <section className="relative min-h-screen flex items-center justify-center overflow-hidden bg-brand-bg">
        ...
        <div className="relative z-10 text-center max-w-4xl mx-auto px-6 lg:px-10 pt-24 pb-36">
          ...
          {/* Headline */}
          <h1 className="font-heading font-bold text-brand-text leading-[1.04] text-[2.5rem] sm:text-[3.5rem] md:text-[4.5rem] lg:text-[5.5rem] mb-8 tracking-[-0.01em]">
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            ...
            <Link
              href="/about"
              className="px-10 py-4 bg-transparent text-brand-text font-body text-[11px] font-normal tracking-[0.22em] uppercase border border-white/15 hover:border-white/35 transition-all duration-200"
            >
              Explore Heritage →
            </Link>
          </div>
        ...
        <div className="absolute bottom-8 left-1/2 -translate-x-1/2 flex flex-col items-center gap-2">
          <span className="font-body text-[9px] tracking-[0.35em] uppercase text-brand-muted/40">
            Scroll
          </span>
          <div
            className="w-px h-10"
            style={{ background: "linear-gradient(to bottom, rgba(248,250,252,0.2), transparent)" }}
          />
        </div>
      </section>
      ...
      <section className="bg-brand-surface border-t border-white/[0.06] py-20 lg:py-24">
        <div className="max-w-7xl mx-auto px-6 lg:px-8">
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-10 lg:gap-16">
            {pillars.map((p) => (
              <div key={p.num} className="border-t border-white/[0.08] pt-8">
      ...
      {/* ── Final CTA ─────────────────────────────────────────────────────────── */}
      <section className="bg-brand-bg border-t border-white/[0.06] py-24 lg:py-32">
        <div className="max-w-3xl mx-auto px-6 text-center">
          ...
          <h2 className="font-heading font-bold text-brand-text text-[1.75rem] sm:text-4xl leading-tight mb-5">
