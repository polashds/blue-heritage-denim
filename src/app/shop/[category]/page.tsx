...
export async function generateMetadata({
  ...
  return {
    title: `${cat.name} — Shop`,
    description: `Shop ${cat.name.toLowerCase()} denim from Blue Heritage Denim.`,
  };
}
...
export default async function CategoryPage({
  ...
  const [products, total, allSizes] = await Promise.all([
    getProducts(opts),
  ...
  return (
    <div className="max-w-7xl mx-auto px-6 lg:px-8 py-12 lg:py-16">
      ...
      <nav className="flex items-center gap-2 font-body text-[10px] tracking-[0.2em] uppercase text-brand-muted mb-8">
        ...
        <span>/</span>
        <span className="text-brand-text">{cat.name}</span>
      </nav>
